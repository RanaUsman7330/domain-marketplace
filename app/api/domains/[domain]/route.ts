// File: /app/api/admin/domains/[domain]/route.ts - UNIFIED API
...
export async function GET(
  ...
  try {
    ...
    // Build query based on whether it's numeric ID or domain name
    let query: string
    let paramsArray: any[]
    
    if (isNumericId(domainIdentifier)) {
      // Search by ID
      query = `
        SELECT 
          d.id,
        ...
        LIMIT 1
      `
      paramsArray = [parseInt(domainIdentifier)]
    } else {
      // Search by name
      query = `
        SELECT 
...
export async function PUT(
  ...
  try {
    ...
    // First, find the domain by identifier
    const domainIdentifier = decodeURIComponent(params.domain)
    
    // Get domain ID first
    let domainId: number
    if (isNumericId(domainIdentifier)) {
      domainId = parseInt(domainIdentifier)
    } else {
